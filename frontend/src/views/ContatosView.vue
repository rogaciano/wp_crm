        ...
        <div class="hidden md:block">
          <table class="table-fixed w-full">
            <thead class="bg-gray-50/50">
              <tr>
                <th class="table-header w-[25%]">Identificação</th>
                <th class="table-header w-[20%]">Contato</th>
                <th class="table-header w-[15%]">Empresa</th>
                <th class="table-header w-[15%]">Opp / Tags</th>
                <th class="table-header w-[13%]">Origem</th>
                <th class="table-header text-right w-[12%]">Ações</th>
              </tr>
            </thead>
              ...
              <tr v-for="contato in contatos" :key="contato.id" class="hover:bg-primary-50/30 transition-colors group">
                <td class="table-cell">
                  <div class="flex items-center gap-2">
                    <div class="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center font-black text-gray-400 text-sm flex-shrink-0 shadow-sm transition-transform group-hover:scale-110">
                      {{ contato.nome.charAt(0).toUpperCase() }}
                    </div>
                    <div class="min-w-0">
                      <div 
                        @click="viewContato(contato.id)"
                        class="font-black text-gray-800 hover:text-primary-600 cursor-pointer transition-colors truncate"
                      >
                        {{ contato.nome }}
                      </div>
                      <div class="text-[10px] text-gray-400 font-bold uppercase tracking-tighter mt-0.5 truncate">
                ...
                <td class="table-cell text-gray-500">
                  <div v-if="contato.email" class="flex items-center gap-1.5 mb-1">
                    <svg class="w-3.5 h-3.5 text-gray-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                    <a :href="`mailto:${contato.email}`" class="text-xs hover:text-primary-600 truncate">{{ contato.email }}</a>
                  </div>
                  <div class="flex items-center gap-1.5 text-[11px] font-bold">
                    ...
                    <span class="truncate">{{ contato.celular_formatado || contato.telefone_formatado || 'Sem telefone' }}</span>
                  </div>
                </td>
                <td class="table-cell">
                  <div v-if="contato.conta_nome" class="min-w-0">
                    <button 
                      @click="router.push(`/contas/${contato.conta}`)"
                      class="text-xs font-black text-gray-700 hover:text-primary-600 flex items-center gap-1.5 max-w-full"
                    >
                      <svg class="w-3.5 h-3.5 opacity-30 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M21 13v10h-6v-6h-6v6h-6v-10l9-9z"/></svg>
                      <span class="truncate">{{ contato.conta_nome }}</span>
                    </button>
                  </div>
                ...
                <td class="table-cell">
                  <div class="flex items-center gap-2">
                    <div class="flex flex-col items-center flex-shrink-0" v-if="contato.oportunidades?.length">
                    ...
                    <div class="flex flex-wrap gap-1 min-w-0">
                      <span 
                        ...
                        class="px-1.5 py-0.5 rounded bg-gray-100 text-[9px] font-bold text-gray-500 uppercase truncate max-w-[70px]"
                      >
                        {{ tag.nome }}
                      </span>
                ...
                <td class="table-cell">
                   <div v-if="contato.tipo_contato_nome" class="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-gray-50 text-[10px] font-black text-gray-600 uppercase max-w-full">
                     <span class="flex-shrink-0">{{ contato.tipo_contato_emoji }}</span>
                     <span class="truncate">{{ contato.tipo_contato_nome }}</span>
                   </div>
                   <div class="text-[9px] text-gray-400 font-bold mt-1 flex items-center gap-1">
                     ...
                     <span class="truncate">{{ contato.canal_nome || 'Sem Canal' }}</span>
                   </div>
                </td>
                 <td class="table-cell text-right">
                   <div class="flex justify-end gap-0.5">
                     <button @click.stop="openWhatsapp(contato)" class="p-1.5 text-emerald-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg" title="WhatsApp">
                     ...
                     <button @click.stop="openModal(contato)" class="p-1.5 text-gray-400 hover:text-primary-600" title="Editar">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                     </button>
                     <button @click.stop="deleteContato(contato.id)" class="p-1.5 text-gray-400 hover:text-red-500" title="Excluir">
