  ...
  <div class="min-h-screen bg-zinc-50 flex font-sans">
    ...
    >
      ...
      <nav class="flex-1 overflow-y-auto px-4 py-6 space-y-1">
        ...
        <!-- Atendimento (sempre visível) -->
        <p class="px-2 text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-2">Atendimento</p>
        <router-link to="/atendimento" class="nav-item" active-class="active">
          <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.019 3.287l-.539 2.016 2.041-.534c.945.512 1.99.782 3.245.782 3.181 0 5.766-2.587 5.768-5.766 0-3.181-2.587-5.766-5.866-5.751zm3.387 7.464c-.135-.067-.807-.399-.933-.444-.124-.045-.215-.067-.306.067-.09.135-.352.444-.43.534-.08.09-.158.101-.293.034-.135-.067-.57-.209-1.085-.67-.399-.356-.67-.795-.749-.933-.08-.135-.011-.202.056-.27.06-.06.135-.158.203-.237.067-.08.09-.135.135-.225.045-.09.022-.169-.011-.237-.034-.067-.306-.745-.421-.998-.103-.236-.211-.201-.306-.201h-.26c-.09 0-.237.034-.361.169s-.474.464-.474 1.13c0 .665.485 1.307.553 1.398.067.09.954 1.458 2.312 2.044.323.139.575.221.77.283.325.103.621.088.854.054.26-.039.807-.33 1.019-.648.214-.318.214-.593.15-.648z"/>
          </svg>
          <div class="flex-1 flex justify-between items-center">
            <span>Multiatendimento</span>
            <span v-if="whatsappUnread.total > 0" class="px-1.5 py-0.5 rounded-sm bg-emerald-600 text-white text-[10px] font-bold">
              {{ whatsappUnread.total }}
            </span>
          </div>
        </router-link>

        <!-- Comercial (visível se hasVendasAccess) -->
        <div v-if="authStore.hasVendasAccess" class="pt-5">
          <p class="px-2 text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-2">Comercial</p>
          ...
          <router-link to="/kanban" class="nav-item" :class="{ 'active': $route.path === '/kanban' && !$route.query.tipo }">
          ...
          <router-link to="/evento" class="nav-item" active-class="active">
        ...
        <!-- Clientes (visível se hasClientesAccess) -->
        <div v-if="authStore.hasClientesAccess" class="pt-5">
          <p class="px-2 text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-2">Clientes</p>

          <router-link to="/kanban?tipo=POS_VENDA" class="nav-item" :class="{ 'active': $route.query.tipo === 'POS_VENDA' }">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
            <span>Clientes Pós-Venda</span>
          </router-link>

          <router-link to="/kanban?tipo=SUPORTE" class="nav-item" :class="{ 'active': $route.query.tipo === 'SUPORTE' }">
          ...
          <router-link to="/onboardings" class="nav-item" active-class="active">
          ...
          <router-link to="/agenda-treinamento" class="nav-item" active-class="active">
            ...
            <span>Agenda Treinamento</span>
          </router-link>
        </div>

        <!-- Dados (sempre visível — compartilhado entre Comercial e Clientes) -->
        <div class="pt-5">
          <p class="px-2 text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-2">Dados</p>
        ...
        <div v-if="isGestor" class="pt-6">
          ...
          <router-link to="/meu-canal/mapa" class="nav-item" active-class="active">
        ...
        <div v-if="isAdmin" class="pt-6">
          ...
          <router-link to="/config/tags" class="nav-item" active-class="active">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
          ...
          <router-link to="/config/modulos-treinamento" class="nav-item" active-class="active">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
            <span>Módulos Treinamento</span>
          </router-link>
          ...
          <router-link to="/config/mapa" class="nav-item" active-class="active">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" /></svg>
    ...
    <main class="flex-1 min-h-screen transition-all duration-300 pt-16 lg:pt-0 overflow-visible">
      <!-- Rotas com layout full-height (sem padding) -->
      <div v-if="$route.name === 'atendimento'" class="h-screen lg:h-screen pt-16 lg:pt-0">
        <router-view />
      </div>
      <!-- Rotas normais (com padding) -->
